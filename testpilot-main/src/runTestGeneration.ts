...
(async () => {
  ...
  // Go up two directories from testpilot-main/dist/ to reach the root project directory
...
function triangle(s1, s2, s3) {
  ...
  const triangleGenerator = new TestGenerator(
    temperatures,
    (fn: string) => triangleSnippets.get(fn),
    model,
    validator,
    collector
  );
...
function isPrime(num) {
  ...
  const isPrimeGenerator = new TestGenerator(
    temperatures,
    (fn: string) => isPrimeSnippets.get(fn),
    model,
    validator,
    collector
  );
...
function findPrimes(lowerBound, upperBound) {
  ...
  const findPrimesGenerator = new TestGenerator(
    temperatures,
    (fn: string) => findPrimesSnippets.get(fn),
    model,
    validator,
    collector
  );
...
function computeSumOfPrimes(primes) {
  ...
  const computeSumGenerator = new TestGenerator(
    temperatures,
    (fn: string) => computeSumSnippets.get(fn),
    model,
    validator,
    collector
  );
  ...
  testInfos.forEach((test: any, index: number) => {
  ...
  // Summary statistics
  const passed = testInfos.filter((t: any) => t.outcome.status === "PASSED").length;
  const failed = testInfos.filter((t: any) => t.outcome.status === "FAILED").length;
